...
categories:
- Linux
last-updated-date: '2021-09-26 20:39:51+08:00'
layout: post
---
...
```bash
find . -type f -exec <command> \; # 一些不要文件参数的命令可以这样执行，如：pwd
find . -type f -exec <command> {} \;
find . -type f -exec <command> {} +
find . -type f -execdir <command> {} +
```

`-exec <command> {} +`：对找到的每个文件执行指定命令，`{}`代表当前的文件名，`+`表示在后面追加所有找到的文件，执行命令的目录是调用`find`的目录，存在[安全隐患](https://www.gnu.org/software/findutils/manual/html_node/find_html/Race-Conditions-with-_002dexec.html)（没看太懂）
...
- 通过管道，截取某（几）个值（空格分界），保留行数

`[command] | awk '{print $3}'`：截取`command`指令输出的第`3`个值（从1开始）
